...
type StudentPage = "home" | "lessons" | "progress" | "schedule" | "support" | "ai-mentor" | "profile" | "settings"
...
export default function StudentDashboard() {
  const user = { name: "Alex Student", avatar: "/placeholder.svg" }
  ...
  const continueSubject = (subjectId: string) => {
    const subject = subjects.find((s) => s.id === subjectId)
    if (subject && subject.unlocked) {
      ...
      setTimeout(() => {
        ...
        setTotalXP((prev) => setTotalXP((prev) => prev + 15))
        toast({
          title: "📈 Progress Updated!",
          description: "You earned 15 XP for studying!",
        })
      }, 2000)
    }
  }
  ...
  const navigationItems = [
    ...
    { id: "lessons", label: "Lessons & Games", icon: Gamepad2 },
